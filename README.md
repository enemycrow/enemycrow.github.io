...
- `imagen_vertical`: ruta relativa dentro del repositorio (por ejemplo `assets/images/social/soledadenelarroyo-post97-portrait.png`) a la versión vertical en formato `.png`. Si no existe, deja una cadena vacía. Estas imágenes solo se usan para automatizaciones externas y no se muestran en el sitio.
...
> Nota: Las variantes verticales se guardan siempre en `assets/images/social/` con extensión `.png`. Ese directorio queda excluido del comando `npm run responsive` para preservar la calidad 9:16 exigida por redes sociales.
...
- `imagen_vertical`: ruta relativa dentro del repositorio a la variante vertical en formato `.png` (por ejemplo `assets/images/social/sunfelicity-fortune50.png`). Usa una cadena vacía cuando no exista. Se reserva para difusión en redes sociales.
